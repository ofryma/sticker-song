 ...
 * says so plainly: the record is kept, and a person will read it through. There
 * is nothing to look at yet either way, so the ways onward are another sticker
 * or the wall itself.
 */
export function Thanks({ entry, onAnother, awaitingReview = false }) {
  const { t } = useI18n();
    ...
    <div className="flex min-h-0 flex-1 flex-col max-sm:overflow-y-auto">
      <div className="m-auto flex flex-col items-center gap-6 py-8 text-center animate-fade-slow sm:gap-8 sm:py-16">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:gap-4">
          <Action onPress={onAnother}>{t("contribute.thanksAnother")}</Action>
        </div>
