...
export function EntryDetail({ entry, onClose, onPrev, onNext }) {
  ...
  const { hasLeaf, add } = useLeaves();

  const open = Boolean(entry);
  const leafAdded = open && hasLeaf(entry.id);
  ...
  return (
    ...
    >
      <ModalContent onKeyDown={onKeyDown}>
        {entry && (
          <ModalBody>
            <div className="sticky top-0 z-10 flex items-center justify-between border-b border-day-line/70 bg-day-soft/95 px-2 py-1.5 backdrop-blur-sm">
            ...
            <div className="grid md:grid-cols-[minmax(0,1.05fr)_minmax(0,1fr)]">
              <div className="bg-day md:border-e md:border-day-line/70">
                <img
                  src={imageUrl(entry)}
              ...
              <EntryDetailBody
                ...
                gesture={
                  <Action
                    tone={leafAdded ? "leaf" : "ghost"}
                    onPress={() => add(entry.id)}
                    isDisabled={leafAdded}
                    className="w-full disabled:opacity-100 sm:w-auto"
                    startContent={<Sprig size={13} open={leafAdded} />}
                  >
